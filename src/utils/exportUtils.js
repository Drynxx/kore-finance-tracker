...
export const exportToCSV = (transactions, currency) => {
    ...
    try {
        ...
        saveAs(blob, `wealtify_transactions_${new Date().toISOString().split('T')[0]}.csv`);
...
export const exportToPDF = (transactions, currency, user) => {
    ...
    try {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text('Generated by Wealtify', 14, 290);
        }

        // Save using file-saver for consistency
        const pdfBlob = doc.output('blob');
        saveAs(pdfBlob, `wealtify_report_${new Date().toISOString().split('T')[0]}.pdf`);
