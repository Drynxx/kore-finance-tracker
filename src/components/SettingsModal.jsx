    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                {/* Global Header */}
                <div className="flex-none px-8 py-6 flex items-center justify-between border-b border-white/5 bg-white/5 relative z-20">
                ...
                {/* Top Navigation Tabs */}
                <div className="flex-none px-8 pt-6 pb-2 relative z-20">
                    <div className="flex p-1 bg-black/20 rounded-2xl border border-white/5">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className="relative flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-xl transition-all duration-300"
                            >
                                {activeTab === tab.id && (
                                    <motion.div
                                        layoutId="activeTab"
                                ...
                                <span className={`relative z-10 flex items-center gap-2 ${activeTab === tab.id ? 'text-white' : 'text-slate-400 hover:text-slate-200'}`}>
                                    <tab.icon size={16} />
                                    {tab.label}
                                </span>
                            </button>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {activeTab === 'appearance' && (
                                <div className="space-y-8">
                                    ...
                                    <section className="space-y-4">
                                        <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider pl-1">Wallpaper Rotation</h4>
                                        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center justify-between hover:bg-white/10 transition-colors">
                                            <div className="flex items-center gap-4">
                                                <div className={`p-3 rounded-xl ${isAutoRotating ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-800 text-slate-500'}`}>
                                                    <RefreshCw size={24} className={isAutoRotating ? 'animate-spin-slow' : ''} />
                                                </div>
                                                <div>
                                                    <h5 className="text-base font-semibold text-white">Auto-Rotate Wallpaper</h5>
                                                    <p className="text-sm text-slate-400">Change background every minute</p>
                                                </div>
                                            </div>

                                            <button
                                                onClick={toggleAutoRotation}
                                                className={`w-14 h-8 rounded-full transition-colors duration-300 relative ${isAutoRotating ? 'bg-indigo-500' : 'bg-slate-700'}`}
                                            >
                                                <div className={`absolute top-1 left-1 w-6 h-6 rounded-full bg-white shadow-md transition-transform duration-300 ${isAutoRotating ? 'translate-x-6' : 'translate-x-0'}`} />
                                            </button>
                                        </div>
                                    </section>
                                    ...
                                    {/* Wallpapers Grid */}
                                    <section className="space-y-4">
                                        <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider pl-1">Select Wallpaper</h4>
                            ...
                            {activeTab === 'general' && (
                                <div className="space-y-8">
                                    <section className="space-y-4">
                                        <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider pl-1">Currency Preference</h4>
                                        <div className="grid grid-cols-1 gap-3">
                                            {Object.values(currencies).map((curr) => (
                                                <button
                                                    key={curr.code}
                                                    onClick={() => changeCurrency(curr.code)}
                                                    className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${currency.code === curr.code
                            ...
                            {activeTab === 'account' && (
                                <div className="space-y-8">
                                    <section className="space-y-4">
                                        <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider pl-1">Profile Information</h4>
                                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center gap-6">
                                            ...
                                            <div>
                                                <h4 className="text-xl font-bold text-white mb-1">{user?.name || 'User'}</h4>
                                                <p className="text-slate-400 font-medium">{user?.email}</p>
                                                <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold border border-emerald-500/20">
                                    ...
                                    <section className="space-y-4">
                                        <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider pl-1">Danger Zone</h4>
                                        <button
                                            onClick={logout}
                                            className="w-full p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-all duration-300 flex items-center justify-between group"
                                        >
                                            <div className="flex items-center gap-3">
                                                <LogOut size={20} />
